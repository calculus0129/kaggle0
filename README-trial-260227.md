...
kaggle.com/learn 사이트에 나온 강좌를 토대로 ML의 기초지식을 습득하고 예시 문제를 해결하는 과정에서 얻게 된 통찰을 공유하는 것을 목표로 한다.
...
- 머신러닝 기술 사용이 장려되는 것으로 보인다.
    ...
    - 회사: AI 경진대회
- 최근 AI/데이터 활용 사례는 증가하고 있으나, 정형 데이터 문제를 **직접 모델링하고 검증해본 경험**은 부족했다.
- Kaggle의 교육 과정과 연습 대회를 활용해 ML을 단순 개념 이해가 아닌 **실험 기반 문제 해결 경험**으로 체득하고자 한다.
...
실제 데이터 문제에 ML(머신러닝) 기술을 적용해보며 모델 선택, 데이터 품질, 평가 기준 등 현실적인 활용 판단 능력을 기르기 위함. 또한 문제를 풀면서 얻게 된 유용한 사고 및 접근 과정을 정리해두고 공유함으로서 나 뿐만 아니라 청중들이 유사한 문제 상황에서 적용할 수 있을 것.

### 🏁 Goal (목표)

* **ML 파트:** Tabular 데이터 기반 'Getting Started' 대회 상위 20% 수준의 성능 도달.
* 얻게 된 Insight(IDK how to abstractly specify this) 정리.
...
- Pandas
- Intermediate ML (US 집값 예측 연습대회를 매 차시마다 exercise로 나감)
...
여기에서 주로 exercise를 하며 'Notebook 기반 실험 및 결과 정리' 과정에 초점을 맞춰 얻은 통찰 포인트를 정리해볼 수 있겠다.
